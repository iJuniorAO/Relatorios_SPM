...
FINANCIAL_CONDITIONS = [
    ...
    lambda df: df["Operação (Nome)"].str.contains(
        "SAÍDA PARA DESPESA|USO E CONSUMO", case=False, na=False
    ),
    lambda df: (df["Operação (Tipo)"] == "Saída")
    & (df["Operação (Nome)"].str.contains("DEVOLUÇÃO|DEVOLUCAO", case=False, na=False)),
    lambda df: (df["Operação (Tipo)"] == "Entrada")
    & (df["Operação (Nome)"].str.contains("DEVOLUÇÃO DE VENDA", case=False, na=False)),
    lambda df: (df["Operação (Tipo)"] == "Saída")
    & (
    ...
    & (
        df["Operação (Nome)"].str.contains(
            "DOACAO|BONIFICACAO|BONIFICAÇÃO|ENTRADA DE INVENTÁRIO", case=False, na=False
        )
    ),
]
...
def check_authentication():
    ...
    if (
        ...
        or st.session_state.perfil.get("status") != "ativo"
        or st.session_state.perfil.get("role") not in REQUIRED_ROLES
    ):
...
def create_losses_chart(df, df_completo):
    ...
    def verDetalhe(df_temp, colunas_remover):
        ...
        df_detalhe = (
            df_detalhe.groupby(
                ["Referência", "CategoriaFinanceira", "Cliente/Fornecedor"]
            )["Total"]
            .sum()
            .reset_index()
        )
...
def create_montly_stores_sales_return(df):
    ...
    df_group = (
        df_filtered.groupby(
            ["Referência", "Cliente/Fornecedor", "CategoriaFinanceira"]
        )["Total"]
        .sum()
        .unstack(fill_value=0)
        .reset_index()
...
with st.sidebar:
    ...
    st.download_button(
        label="Download DataFrame", data=csv, file_name="dataframe.csv", mime="text/csv"
    )
...
mes_selecionado = st.segmented_control(
    "Selecione o Mês", options=meses, default=meses[-1]
)
